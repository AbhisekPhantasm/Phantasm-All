...
test('About Us Page Comprehensive QA Scenario', async ({ page }, testInfo) => {
  const pageName = 'About Us Page';
  const url = 'https://ushodaya.phantasm.solutions/about-us/';
  ...
  console.log(`Navigating to About Us: ${url}`);
  await page.goto(url, { waitUntil: 'domcontentloaded' });
  await page.waitForTimeout(1000);
  await snap('1 - About Us Page Loaded');
  ...
  await testInfo.attach('2 - About Us Full Page', { body: fullSs, contentType: 'image/png' });
  ...
  await snap('3 - About Us Footer Section');
