...
test('Learning Facilities Page Comprehensive QA Scenario', async ({ page }, testInfo) => {
  const pageName = 'Learning Facilities';
  const url = 'https://ushodaya.phantasm.solutions/learning-facilities/';
  const bugs: any[] = [];
  ...
  await snap('1 - Learning Facilities Page Loaded');
  ...
  await testInfo.attach('2 - Learning Facilities Full Page (After Scroll)', { body: fullSs, contentType: 'image/png' });
  ...
  await snap('3 - Learning Facilities Footer Section');
