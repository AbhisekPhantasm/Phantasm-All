...
test('Home Page Comprehensive QA Scenario', async ({ page }, testInfo) => {
  ...
  await testInfo.attach('2 - Home Full Page (After Scroll)', { body: fullSs, contentType: 'image/png' });

  // NAVBAR ITEMS
  console.log('Testing Navbar items...');
  const navItems = ['Academics', 'About Us', 'Programs', 'Gallery', 'Blogs', 'Contact Us'];
  for (const item of navItems) {
    const navLink = page.getByRole('link', { name: item, exact: true }).first();
    if (await navLink.isVisible()) {
      console.log(`- Clicking Navbar: ${item}`);
      await navLink.hover();
      await navLink.click();
  ...
  await snap('3 - Home Footer Section');
