...
test.describe('Novexiz Page — Comprehensive Test', () => {
  ...
  test('Novexiz Comprehensive Test - All Scenarios', async ({ page }, testInfo) => {
    test.setTimeout(120000); // Comprehensive test needs more than the default 60s
    await login(page);
    
    // 1. Navigate to Novexiz page and verify it loads
    await page.goto(`${BASE_URL}/novexiz`);
    await page.waitForLoadState('domcontentloaded', { timeout: 30000 });
    await page.waitForTimeout(600);
    
    const heading = page.locator('h1, h2, h3, [class*="heading"], [class*="title"]').first();
    await expect(heading).toBeVisible({ timeout: 10000 });
    await expect(page).toHaveURL(/novexiz/);
    await page.screenshot({ path: 'screenshots/novexiz-01-page-load.png', fullPage: true });
    ...
    await page.screenshot({ path: 'screenshots/novexiz-02-header.png', fullPage: true });
    ...
    await page.screenshot({ path: 'screenshots/novexiz-03-banner.png', fullPage: true });
    
    // 4. Scroll Through Page
    await page.evaluate(() => window.scrollTo(0, document.body.scrollHeight * 0.5));
    await page.waitForTimeout(800);
    await page.screenshot({ path: 'screenshots/novexiz-04-scroll.png', fullPage: true });
    ...
    await page.screenshot({ path: 'screenshots/novexiz-05-products.png', fullPage: true });
    await attachBrandOverviewForAllure(page, testInfo, 'novexiz');
    ...
    await page.screenshot({ path: 'screenshots/novexiz-05-product-detail.png', fullPage: true });
    ...
    try {
      ...
      if (await filterSection.isVisible({ timeout: 5000 })) {
        ...
        await page.screenshot({ path: 'screenshots/novexiz-06-filter-section.png', fullPage: true });
      }
    } catch {
    ...
    // 7. FAQ accordion — click each "+" / expand control for all 6 questions (Novexiz)
    const novexizFaqPatterns = [
      /What makes Novexiz different from regular herbal supplements/i,
      /Can I take Novexiz products with other supplements or medications/i,
      /Are Novexiz products safe for long-term use/i,
      /How should I store Novexiz products to maintain their quality/i,
      /Do Novexiz products cause any side effects/i,
      /How long should I use Novexiz products before expecting noticeable results/i,
    ];
    ...
    try {
      ...
      await expect(faqSection.getByText(/What makes Novexiz different/i).first()).toBeVisible({
        timeout: 25000,
      });
      ...
      for (const pattern of novexizFaqPatterns) {
        await clickFaqExpand(pattern);
        await page.waitForTimeout(350);
      }

      await page.screenshot({ path: 'screenshots/novexiz-faq-accordion.png', fullPage: true });
    } catch {
    ...
    await page.screenshot({ path: 'screenshots/novexiz-07-product-detail.png', fullPage: true });
    ...
    await page.screenshot({ path: 'screenshots/novexiz-08-footer.png', fullPage: true });
    
    console.log('✅ Novexiz Comprehensive Test Complete');
  });
});