...
test.describe('Dates Up Page — Comprehensive Test', () => {
  ...
  test('Dates Up Comprehensive Test - All Scenarios', async ({ page }, testInfo) => {
    await login(page);
    
    // 1. Navigate to Dates Up page and verify it loads
    await page.goto('http://82.112.230.248:6022/dates-up');
    await page.waitForLoadState('domcontentloaded');
    await page.waitForTimeout(2000);
    
    const heading = page.locator('h1, h2, h3, [class*="heading"], [class*="title"]').first();
    await expect(heading).toBeVisible({ timeout: 10000 });
    await expect(page).toHaveURL(/dates-up/);
    await page.screenshot({ path: 'screenshots/dates-up-01-page-load.png', fullPage: true });
    ...
    await page.screenshot({ path: 'screenshots/dates-up-02-header.png', fullPage: true });
    ...
    await page.screenshot({ path: 'screenshots/dates-up-03-banner.png', fullPage: true });
    
    // 4. Scroll Through Page
    await page.evaluate(() => window.scrollTo(0, document.body.scrollHeight * 0.5));
    await page.waitForTimeout(800);
    await page.screenshot({ path: 'screenshots/dates-up-04-scroll.png', fullPage: true });
    ...
    await page.screenshot({ path: 'screenshots/dates-up-05-products.png', fullPage: true });
    await attachBrandOverviewForAllure(page, testInfo, 'dates-up');
    ...
    await page.screenshot({ path: 'screenshots/dates-up-05-product-detail.png', fullPage: true });
    ...
    try {
      ...
      if (await filterSection.isVisible({ timeout: 5000 })) {
        ...
        await page.screenshot({ path: 'screenshots/dates-up-06-filter-section.png', fullPage: true });
      }
    } catch {
    ...
    // 7. FAQ accordion — click each "+" / expand control for all 6 questions (DatesUp)
    const faqQuestionPatterns = [
      /What makes DatesUp different from regular cola or energy drinks/i,
      /Is DatesUp safe for daily consumption/i,
      /Can kids drink DatesUp/i,
      /What are DatesUp functional sachets used for/i,
      /Are DatesUp products made with artificial sweeteners or colors/i,
      /How do I consume DatesUp sachets/i,
    ];
    ...
    try {
      ...
      await expect(faqSection.getByText(/What makes DatesUp different/i).first()).toBeVisible({
        timeout: 25000,
      });
      ...
      await page.screenshot({ path: 'screenshots/dates-up-faq-accordion.png', fullPage: true });
    } catch {
    ...
    await page.screenshot({ path: 'screenshots/dates-up-07-product-detail.png', fullPage: true });
    ...
    await page.screenshot({ path: 'screenshots/dates-up-08-footer.png', fullPage: true });
    
    console.log('✅ Dates Up Comprehensive Test Complete');
  });
});