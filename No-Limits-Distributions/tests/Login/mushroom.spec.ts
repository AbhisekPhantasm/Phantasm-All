...
test('Shop – Mushroom Category Page Validation', async ({ page }) => {
  test.setTimeout(120000);

  // ── STEP 1: Login to Website ──
  await loginToSite(page);
  await snap(page, 'mushroom_01_logged_in');
  ...
  // ── STEP 2: Navigate to Mushroom Category Page ──
  await page.goto('https://nldus.com/shop?category=mushrom');
  await page.waitForLoadState('networkidle');
  await page.waitForTimeout(3000);
  await dismissAgeGate(page);
  await expect(page).toHaveURL(/mushrom/);
  await snap(page, 'mushroom_02_page_loaded');
  console.log('✅ Step 2: Mushroom category page loaded successfully');
  ...
  await snap(page, 'mushroom_03_heading_visible');
  ...
  await snap(page, 'mushroom_04_empty_state_checked');
  ...
  await snap(page, 'mushroom_05_product_container_checked');
  ...
  await snap(page, 'mushroom_06_filter_sidebar_checked');
  ...
  await snap(page, 'mushroom_07_page_scrolled');
  console.log('✅ Step 7: Mushroom page scrolled successfully');
  ...
  await snap(page, 'mushroom_08_footer_visible');
  ...
  await page.waitForLoadState('networkidle');
  await page.waitForTimeout(2000);
  await snap(page, 'mushroom_09_navigated_back');
  console.log('✅ Step 9: Navigated back to homepage');

  // ── STEP 10: Final Validation ──
  const title = await page.title();
  expect(title).not.toBe('');
  await snap(page, 'mushroom_10_final_validation');
  console.log('✅ Step 10: Mushroom category page test completed successfully');
});
